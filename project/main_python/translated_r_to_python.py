...
def convert_csv_to_xyz_df(csv_filename):
    xyz_df=fr.csv_filename_to_dataframe(csv_filename,columns=xyz_lib.XYZConstants.DF_COLUMNS.value)
    xyz_df.replace({xyz_lib.XYZConstants.DF_COLUMNS.value[0]: GeneralConstants.ATOMIC_NUMBERS.value},inplace=True)
    return xyz_df
